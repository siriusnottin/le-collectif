...
export default function InterieurQuestion({ options, isSelected, onSelect }: QuestionComponentProps) {
  return (
          ...
          <button
            ...
            className={`relative overflow-hidden rounded-lg transition-all duration-300 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-collective-accent/50 group ${
              isSelected(option.value)
                ? 'ring-4 ring-collective-accent shadow-soft-lg transform scale-105'
            ...
            {option.image ? (
              <div className="relative overflow-hidden">
                ...
                <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/70 to-transparent p-4">
                  <p className="font-sans text-white font-medium">{option.label}</p>
                </div>
              </div>
            ) : null}
          </button>
